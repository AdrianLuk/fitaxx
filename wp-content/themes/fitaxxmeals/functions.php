<?php
define('WPFORGE_VERSION', '6.5.3.1');
...
if (!function_exists('wpforge_adjust_content_width')) {
    function wpforge_adjust_content_width()
    {
        ...
        if (
            is_page_template('full-width.php') || is_page_template('front-page.php') || is_attachment() ||
    ...
    function wpforge_comment($comment, $args, $depth)
{
        ...
        switch ($comment->comment_type): case 'pingback':
    case 'trackback':
    ...
    function wpforge_entry_meta_header()
{
        ...
        printf(
            '<span class="entry-date updated"><span class="genericon genericon-month"></span><a href="%1$s" rel="bookmark"><time class="entry-date" datetime="%2$s">%3$s</time></a></span> <span class="byline"><span class="genericon genericon-user"></span><span class="author vcard"><a class="url fn n" href="%4$s" rel="author">%5$s</a></span></span>',
...
if (function_exists('acf_add_options_page')) {
    $page = acf_add_options_page(array(
        'page_title' => __('CTA Options'),
        'menu_title' => __('Call to Action'),
        'menu_slug' => 'cta-options',
        'capability' => 'edit_posts',
        'redirect' => false,
    ));
    acf_add_options_page(array(
        'page_title' => 'Delivery Options',
        'menu_title' => 'Delivery Options',
        'menu_slug' => 'delivery-options',
...
function posts_json($data, $post, $context)
{
    $title = get_the_title($post->ID);
    $thumbnail = get_the_post_thumbnail_url($post->ID);
    if ($title) {
        $data->data['post_title'] = $title;
    }
    $data->data['thumbnail'] = $thumbnail;
    return $data;
}
add_filter('rest_prepare_meats', 'posts_json', 10, 3);
add_filter('rest_prepare_carbs', 'posts_json', 10, 3);
add_filter('rest_prepare_vegetables', 'posts_json', 10, 3);
add_filter('rest_prepare_snacks', 'posts_json', 10, 3);
add_filter('rest_prepare_salads', 'posts_json', 10, 3);
add_filter('rest_prepare_vegan', 'posts_json', 10, 3);
add_filter('rest_prepare_fall_menu', 'posts_json', 10, 3);
add_filter('rest_prepare_winter_menu', 'posts_json', 10, 3);
add_filter('rest_prepare_juice', 'posts_json', 10, 3);
add_filter('rest_prepare_smoothie', 'posts_json', 10, 3);

// Make a function to add our CSS links to the site
function fitaxx_enqueue_styles()
{
    // dynamically look for required css files for order page
    foreach (glob("./wp-content/themes/fitaxxmeals/page-templates/static/css/*.css") as $filename) {
        $file = explode("/", $filename)[count(explode("/", $filename)) - 1];
        wp_enqueue_style(
            'fitaxxstyle',
            get_template_directory_uri() . '/page-templates/static/css/' . $file,
            // An array of CSS files to override
            array(),
...
add_action('wp_enqueue_scripts', 'fitaxx_enqueue_styles', 9999);

// Make a function to add our custom JS to the site
function fitaxx_scripts()
{
    // dynamically add required order form js files if order page is detected
    if (is_page('order')) {
        foreach (glob("./wp-content/themes/fitaxxmeals/page-templates/static/js/*.js") as $filename) {
            $file = explode("/", $filename)[count(explode("/", $filename)) - 1];
            wp_enqueue_script(
                $file,
                get_template_directory_uri() . '/page-templates/static/js/' . $file,
                array(),
                '1',
                true
            );
...
function dotiavatar_function()
{
    ...
    switch ($date) {
        case 'Sunday':
            if($time <12){
                $content = "Order now for <strong>Monday</strong> delivery";
            } else {
                $content = "Order now for <strong>Thursday</strong> delivery";
            }
            break;
        case 'Monday':
            $content = "Order now for <strong>Thursday</strong> delivery";
...
add_shortcode('dotirating', 'dotiavatar_function');
